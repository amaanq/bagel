...
//!
//! A scanner that fetches `/.env` expects `KEY=value` text, `/.git/config`
//! expects INI, `/config.json` expects JSON. Serving an HTML page with
//! navigation links (as the Markov+Lua path does) instantly reveals a honeypot.
//! This module synthesizes bodies that match the file the probe asked for, so
//! the leak looks real; the HTML kinds keep using Markov + the Lua maze.
...
pub struct Profile {
    ...
    /// Whether the body is an HTML document. HTML bodies go through Markov and
    /// the Lua crawler-maze; everything else is synthesized here as a plausible
    /// file leak.
    pub html: bool,
    /// Whether to serve this as a static file (adds `Last-Modified`, `ETag`,
    /// and `Accept-Ranges`, exactly as a web server would for a real file).
...
/// Decide how to dress up a `(kind, path)` pair. Path extension wins over the
/// coarse category so `/config.json` is JSON even though it is `env_secrets`.
///
/// A deterministic slice of paths (sized by `not_found_pct` + `forbidden_pct`)
/// is served as `404`/`403` instead of a bait, so the tarpit does not answer
/// every probe with a suspicious `200`. The disposition is keyed on the path,
/// so a given URL is *always* answered the same way. A scanner that retries
/// sees a consistent, real-looking server rather than a flapping one.
#[must_use]
...
fn content_profile(kind: &str, path: &str) -> Profile {
    ...
    match kind {
        "env_secrets" | "vcs_leak" | "path_traversal" => Profile {
            status: OK,
        ...
        // WordPress and bare PHP are served by a PHP stack; that stack sets
...
/// A short, stable tag derived from the interaction `token`, so a credential
/// that later shows up in the operator's logs can be traced to the bot that was
/// handed it, without making the value look synthetic. Uppercase base36, 6
/// chars, embedded inside an otherwise realistic secret.
...
/// A believable-looking secret value tied to `token` so an operator can trace
/// which trapped interaction a leaked credential came from if it is ever used.
/// The tag is spliced into the middle so the value still scans as random.
#[must_use]
...
fn json_leak(path: &str, token: &str) -> String {
    if path.to_ascii_lowercase().contains("containers/json") {
        ...
        return format!(
            "[{{\"Id\":\"{id}\",\"Names\":[\"/app_web_1\"],\"Image\":\"app:latest\",\
             \"Command\":\"docker-entrypoint.sh\",\"State\":\"running\",\
             \"Status\":\"Up 6 days\",\"Ports\":[{{\"PrivatePort\":8080,\"Type\":\"tcp\"}}]}}]"
        );
    }
...
fn xml_leak(token: &str) -> String {
    ...
    format!(
        "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<configuration>\n  \
         ...
         \"Server=127.0.0.1;Database=app;User Id=sa;Password={pw};\" />\n  \
         </connectionStrings>\n  <appSettings>\n    <add key=\"MachineKey\" value=\"{key}\" />\n  \
         </appSettings>\n</configuration>\n"
    )
}
...
mod tests {
    ...
    #[test]
    fn html_kinds_are_marked_html() {
        for kind in ["wordpress", "php", "admin_panel", "cgi_rce", "other"] {
            assert!(
                profile(kind, "/wp-login.php", 0, 0).html,
                "{kind} should be HTML"
            );
        }
    }
    ...
    #[test]
    fn json_config_is_valid_shape() {
        let p = profile("env_secrets", "/app/credentials.json", 0, 0);
        let b = body(&p, "env_secrets", "/app/credentials.json", "TOK");
        assert!(b.trim_start().starts_with('{') && b.trim_end().ends_with('}'));
    }
    ...
    #[test]
    fn error_page_looks_like_nginx() {
        let page = error_page("404 Not Found", "nginx/1.24.0");
        assert!(page.contains("<h1>404 Not Found</h1>"));
        assert!(page.contains("<center>nginx/1.24.0</center>"));
    }
}
