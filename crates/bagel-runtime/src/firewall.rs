//! Atomic nftables enforcement for durable ban leases.
...
impl Scope {
   #[must_use]
   pub const fn all_ports(verdict: Verdict) -> Self {
      Self {
         protocol: None,
         ports: Vec::new(),
         verdict,
   ...
   #[must_use]
   pub const fn protocol(protocol: Protocol, verdict: Verdict) -> Self {
      Self {
...
impl Firewall {
   #[must_use]
   pub const fn new(nft_path: PathBuf, table: String, priority: i32, required: bool) -> Self {
      Self {
         nft_path,
         table,
         priority,
   ...
   pub async fn reconcile(&self, memberships: &[Membership]) -> std::io::Result<()> {
      ...
      let ruleset = render_ruleset(&self.table, self.priority, memberships)?;
...
pub(crate) fn networks_overlap(left: IpNetwork, right: IpNetwork) -> bool {
   if left.is_ipv4() != right.is_ipv4() {
      return false;
   }
   left.contains(right.ip()) || right.contains(left.ip())
}
...
fn write_set(
   ...
   let _ = writeln!(
      output,
      "  set scope_{index}_{family} {{ type {kind}; flags interval; auto-merge; elements = {{ \
       {values} }}; }}"
   );
}
...
mod tests {
   ...
   fn ruleset_keeps_scopes_and_families_separate() {
      ...
      let udp = Scope::protocol(Protocol::Udp, Verdict::Drop);
      let all = Scope::all_ports(Verdict::Drop);
      let rules = render_ruleset("bagel", -10, &[
         Membership {
   ...
   fn redirect_rejects_udp() {
      Scope::new(Protocol::Udp, vec![53], Verdict::Redirect(2222)).unwrap_err();
   }
   ...
   fn blocks_override_redirects_and_redirect_conflicts_fail() {
      ...
      let rules = render_ruleset("bagel", -10, &[drop, redirect.clone()]).unwrap();
      ...
      render_ruleset("bagel", -10, &[redirect, conflicting]).unwrap_err();
   }
}
