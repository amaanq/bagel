      ...
      match detector {
         ...
         Detector::Json {
            ...
            Ok(Self::Json {
               ...
               group_key_pointer: group_key_pointer.clone(),
               timestamp_format:  parse_timestamp_format(timestamp_format.as_deref())?,
            })
         },
      }
   ...
   pub fn detect(
      ...
      match self {
         Self::Regex {
            ...
            let entry = cache.entry(key.to_owned()).or_insert_with(|| {
               ContextEntry {
         ...
         Self::Json {
            ...
            let observed_at = timestamp_pointer
               ...
               .unwrap_or(received_at);
            let group_key = match group_key_pointer {
               Some(pointer) => Some(value.pointer(pointer)?.as_str()?.to_owned()),
               None => None,
            };
            Some(Detection {
               network,
...
fn parse_timestamp(raw: &str, format: &TimestampFormat) -> Option<u64> {
   match format {
      // SQLite INTEGER timestamps are signed, so reject values outside its
      // range before they reach the store.
      TimestampFormat::Unix => {
         let timestamp = raw.parse::<u64>().ok()?;
         i64::try_from(timestamp).is_ok().then_some(timestamp)
      },
...
mod tests {
   ...
   fn regex_prefilter_extracts_literal_ip() {
      let detector = CompiledDetector::new(&Detector::Regex {
         ...
         patterns:            vec![
            r"failed from (?P<address>[0-9.]+) on (?P<attempt_key>/[^ ]+)".into(),
         ],
